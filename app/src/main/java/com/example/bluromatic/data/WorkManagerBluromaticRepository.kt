...
class WorkManagerBluromaticRepository(
    ...
    override fun applyBlur(blurLevel: Int) {
        ...
        // 1 - Limpar imagens temporárias antigas
        ...
        // 2 - Definir restrição para o BlurWorker
        val constraints = Constraints.Builder()
            .setRequiresBatteryNotLow(true)
            .build()

        // 3 - Criar o Worker de desfoque com a restrição
        val blurBuilder =
            OneTimeWorkRequestBuilder<BlurWorker>()
                .setConstraints(constraints)

        // 4 - Enviar os dados de entrada
        ...
        // 5 - Adicionar BlurWorker à cadeia
        continuation = continuation.then(
            blurBuilder.build()
        )

        // 6 - Criar Worker que salva a imagem final
        val save =
            OneTimeWorkRequestBuilder<SaveImageToFileWorker>()
                .build()

        // 7 - Adicionar SaveImageToFileWorker à cadeia
        continuation = continuation.then(save)

        // 8 - Executar toda a cadeia
